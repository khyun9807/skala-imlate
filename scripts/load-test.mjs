#!/usr/bin/env node
/**
 * imlate 부하 테스트 — "등록 마감(기본 22:15) 직전 몰림" 시뮬레이션
 *
...
async function phaseSharedWifi(date, state) {
  ...
  info('  → 여기서 429 가 나오면 마감 직전(기본 22:15)에 정상 교육생이 차단된다는 뜻이다(운영 불가).')
  clearRateLimits()
...
async function phasePreview(date, totalRows) {
  section(`5. 명단 렌더링 — 사감 발송(기본 22:25) 준비 비용 측정 (총 ${totalRows}명)`)
