...
class ReconciliationServiceTest {
    ...
    void 취소된_등록은_되살리지_않는다() {
        ...
        walRows(List.of(TestFixtures.walEntry("wal-1", TestFixtures.DATE, "1", "홍길동", "302")));
    ...
    void 취소분이_섞여도_나머지는_정상_대사한다() {
        ReturnRegistration active = TestFixtures.registration(1L, "1", "김교육", "301");
        ReturnRegistration cancelled = TestFixtures.cancelledRegistration(2L, "1", "홍길동", "302");
        dbRows(List.of(active, cancelled));
        walRows(List.of(
                TestFixtures.walEntry("wal-1", TestFixtures.DATE, "1", "김교육", "301"),
                TestFixtures.walEntry("wal-2", TestFixtures.DATE, "1", "홍길동", "302")));
    ...
    void WAL_전용_항목을_복구한다() {
        WalEntry missing = TestFixtures.walEntry("wal-1", TestFixtures.DATE, "1", "홍길동", "302");
        dbRows(List.of());
    ...
    void 일치하면_CONSISTENT_다() {
        ReturnRegistration row = TestFixtures.registration(1L, "1", "홍길동", "302");
        dbRows(List.of(row));
        walRows(List.of(TestFixtures.walEntry("wal-1", TestFixtures.DATE, "1", "홍길동", "302")));
    ...
    void WAL_사용_불가시_WAL_UNAVAILABLE_이다() {
        dbRows(List.of(TestFixtures.registration(1L, "1", "홍길동", "302")));
    ...
    void inspect_는_복구하지_않는다() {
        ...
        assertThat(report.recoveredCount()).isZero();
        assertThat(report.dbCount()).isZero();
        assertThat(report.walOnly()).containsExactly("1/홍길동/302");
        verify(registrationWriter, never()).recover(any(ReturnRegistration.class), anyBoolean());
    }
    ...
    void DB_전용_항목이_있으면_MISMATCH_다() {
        dbRows(List.of(TestFixtures.registration(1L, "2", "김철수", "410")));
        walRows(List.of());

        ReconciliationReport report = service.reconcile(TestFixtures.DATE);

        assertThat(report.status()).isEqualTo(ReconciliationReport.STATUS_MISMATCH);
        assertThat(report.dbOnly()).containsExactly("2/김철수/410");
        assertThat(report.walOnly()).isEmpty();
    }
    ...
    void 이미_존재하는_항목은_복구하지_않는다() {
        WalEntry entry = TestFixtures.walEntry("wal-9", TestFixtures.DATE, "1", "홍길동", "302");
        // DB 목록 조회에는 잡히지 않지만(정렬 조회 시점 차이) 단건 조회로는 존재하는 상황
        dbRows(List.of());
        walRows(List.of(entry));
        when(registrationRepository.findByRegistrationDateAndClassNameAndStudentNameAndRoomNumber(
                any(), anyString(), anyString(), anyString()))
                .thenReturn(Optional.of(TestFixtures.registration(3L, "1", "홍길동", "302")));
    ...
    void 상태와_무관하게_DB_존재_여부로_판단한다() {
        WalEntry failed = new WalEntry("wal-f", TestFixtures.DATE, "1", "홍길동", "302",
    ...
    void COMMITTED_복구는_통계를_재집계하지_않는다() {
        ...
        WalEntry committed = new WalEntry("wal-c", TestFixtures.DATE, "1", "홍길동", "302",
