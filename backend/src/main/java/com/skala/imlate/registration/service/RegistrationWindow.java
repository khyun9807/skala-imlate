 ...
 *
 * <p><b>등록과 취소는 마감이 다르다</b>(기본 22:15 / 22:20). 그래서 두 벌을 함께 내려보낸다 —
 * 등록 화면은 {@code open}/{@code closesAt} 를, 취소 화면은 {@code cancelOpen}/{@code cancelClosesAt}
 * 를 본다. 한 벌만 내려보내고 프론트가 5분을 더하는 식으로 만들면, 나중에 설정으로 간격을 바꿨을 때
 * 화면만 옛 값을 말하게 된다(서버가 유일한 진실이어야 한다).
 *
 ...
 * @param secondsUntilClose       등록 마감까지 남은 초(이미 지났으면 0)
 * @param cancelOpen              지금 취소 가능한지
 * @param cancelClosesAt          오늘의 취소 마감 시각(이 시각부터 거부)
 * @param secondsUntilCancelClose 취소 마감까지 남은 초(이미 지났으면 0)
 */
