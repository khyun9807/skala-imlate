...
7. **사람이 판단할 잔여물 목록 출력** — 지우지는 않는다(아래 §3).

## 2. 최종 스냅샷을 남기지 않는 이유

화면 공지에 **"그동안 등록된 명단과 개인정보는 서비스 종료와 함께 모두 파기됩니다"** 라고 밝혔다.
...
## 3. 이 계정에 imlate 것이 아닌 자원이 섞여 있다

계정 점검에서 이 프로젝트 소유가 아닌 것으로 보이는 자원을 발견했다.
**스크립트가 자동으로 지우지 않는다.** 다른 프로젝트 자산일 수 있어서다. 직접 확인하고 판단할 것.
...
이것들을 그대로 두면 **imlate 를 다 지워도 계정 청구가 0 이 되지 않는다.**
