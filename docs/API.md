...
```json
{
  ...
  "errors": [
    { "field": "studentName", "message": "이름은 띄어쓰기 없이 한글 또는 영문만 입력해 주세요." }
  ]
}
```
...
| `studentName` | string | 필수, 1~20자, **한글·영문만**. 띄어쓰기 불가 |
...
> **가운데 공백은 어느 필드에도 허용하지 않습니다.** `"Alice Kim"`, `"남궁 민수"` 는 거부되며 붙여서(`"AliceKim"`) 입력해야 합니다.
