...
```json
{
  ...
  "errors": [
    { "field": "studentName", "message": "이름에는 한글·영문·숫자와 공백, 괄호, 하이픈만 사용할 수 있습니다." }
  ]
}
```
...
| 필드 | 타입 | 제약 |
|---|---|---|
| `className` | string | 필수, 1~20자, `^[가-힣A-Za-z0-9 ()\-]{1,20}$` |
| `studentName` | string | 필수, 1~20자, 동일 정규식 |
| `roomNumber` | string | 필수, 1~20자, 동일 정규식 |

> 서버는 `@Valid` 통과 후 다시 **정규화(앞뒤 공백 제거 + 연속 공백 1칸)** 하고 같은 문자 집합으로 재검증합니다.
> 대소문자 변환은 하지 않습니다. `"A-1 (2)"`, `"남궁 민수"` 는 통과, `"<script>"`, 개행, `;` 는 거부됩니다.
