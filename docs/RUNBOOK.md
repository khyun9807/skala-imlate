...
> 그래서 이 런북의 1순위는 **"어떻게든 23:30 일괄 개방 전에 사감님 손에 명단을 쥐여 주는 것"** 입니다.
> 원인 규명은 그다음입니다.
...
00:00 등록 시작 → 22:15 등록 마감 → 22:20 취소 마감 → 22:25 사감 발송
     → 22:30 문 잠김 → 22:35 / 22:45 실패분 자동 재시도 → 23:30 일괄 개방
```

**22:25에 실패해도 일괄 개방(23:30)까지 65분이 남아 있습니다.** 당황하지 말고 §1 순서대로 내려가세요.

> ⚠️ **자동 재시도(22:35 / 22:45)는 문 잠김(22:30) 이후입니다.** 예전에는 통금 전에 끝났지만,
> 등록 마감을 22:15로 늦추면서 자리가 없어졌습니다. 그래서 **재시도를 기다리는 것이
> 더 이상 안전한 선택이 아닙니다** — 22:25 실패 알림을 받으면 바로 §1 로 내려가세요.
> 최종 마지노선은 22:30 이 아니라 **23:30 일괄 개방**입니다.
...
> 그때까지는 **22:27에 사람이 §1.1 을 직접 실행하는 것**이 확실한 감지 수단입니다.

---

## 1. 22:25인데 사감님이 문자를 못 받았다 ★ 최우선
...
| 아무것도 없음 + `imlate` 는 `active` | 시간대 오설정(UTC로 돌면 22:25 KST 를 놓친다) | `timedatectl` 이 `Asia/Seoul` 인지 확인 → §1.8 강제 발송 |
...
| 응답 | 판정 |
|---|---|
| `open: false` + 현재 시각이 22:15 이후 | **정상 동작.** 다음 날 대상 등록은 자정(00:00)에 열립니다 |
| `open: false` 인데 아직 22:15 전 | 서버 시간대 오설정 의심 → `Run-OnApp "timedatectl"` 로 `Asia/Seoul` 확인 |
| `closesAt` 이 22:15 가 아님 | 누군가 `IMLATE_REGISTRATION_CLOSE_TIME` 을 바꿨다 → [OPERATIONS.md §5.2](OPERATIONS.md#52-등록-마감시간-변경-예-2215--2130) |

> 마감 이후 등록을 **예외적으로 받아야 한다면** 마감 시각을 잠깐 늦추는 것이 아니라,
> 사감님께 §1.7 방식으로 개별 연락하는 편이 안전합니다. 마감을 미루면 22:25 발송과 겹칩니다.
...
22:14 처럼 동시 접속이 몰리는 순간 앱에 닿기도 전에 429 가 나갈 수 있습니다.
...
DB 가 죽어 500 이 나도 WAL 에 남아 있고, **22:25 발송 경로의 WAL↔DB 대사가 누락분을 DB 로 복구**합니다.
...
3. **DB 복구 후 반드시 대사 + 발송을 다시 실행**(22:25 이 이미 지났다면)
...
| "등록 마감 시간(22:15)이 지났습니다" | 마감 후 취소 시도 | 정상. 명단은 이미 발송됨 — 사감님께 구두로 전달 |
...
| 알람 이름(접미어) | 뜻 | 이동 |
|---|---|---|
| `-dispatch-heartbeat-missing` | **22:25 발송이 아예 일어나지 않았다** | 아래 §3.1 → 살아나면 **§1.8 강제 발송** |
...
> **22:15~22:30 사이에 온 알람은 "오늘 명단이 안 나갔을 수 있다"는 뜻입니다.**
...
> 발송 전(22:25 이전)에 조회하면 정상적으로도 이렇게 나옵니다. §4.2 로 판정하세요.
...
| **`walOnly` 에 값이 있다** — 아직 발송 전(22:25 이전) | 복구를 아직 안 돌렸을 뿐 | **정상 범위.** 발송 때 복구됩니다. 급하면 아래 복구 재실행 |
...
2. 없는데 본인은 "등록했다"고 한다 → 등록 시각을 물어보세요. **22:15 이후면 등록이 안 된 것**입니다
3. 22:15 이전이라는데 없다 → §4.1 대사 결과 확인 → `walOnly` 에 이름이 있으면 §4.2 복구 재실행
