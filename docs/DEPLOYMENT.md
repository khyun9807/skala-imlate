...
| 야간만 가동(개발 환경) | 인스턴스 시간 비례 | 22:15~22:30(마감·발송·재시도) 시간대는 반드시 켜 둘 것 |
...
- [ ] **22:25 발송 스케줄이 KST 로 도는지** 다음 날 로그로 확인
...
> **22:25 직전에 인스턴스가 죽으면 발송이 통째로 실패하는데, 알람이 없으면 아무도 모릅니다.**
...
|---|---|---|
| `-dispatch-heartbeat-missing` | **22:25 발송이 아예 일어나지 않음**(인스턴스 다운 등) | [RUNBOOK §1.3](RUNBOOK.md#13-이력이-아예-없다-스케줄러-미실행) → 살아나면 강제 발송 |
...
정상 운영일에는 22:25 신호 덕분에 하루 종일 `OK` 이고, 신호가 없는 날에만 22:00 전후에 `ALARM` 으로 전이합니다.
...
> 이 상태에서는 **22:25 발송이 통째로 실패해도 알람이 울리지 않습니다.**
> 알람이 붙기 전과 똑같은 상황이므로, 해결 전까지는
> [OPERATIONS.md §1.1](OPERATIONS.md#11-일일-운영-체크리스트) 의 22:27 수동 확인을 계속하세요.
...
특히 **22:15~22:30 사이에 온 알람은 "발송이 안 됐을 수 있다"는 뜻**이므로,
원인 규명보다 [RUNBOOK §1](RUNBOOK.md#1-2225인데-사감님이-문자를-못-받았다--최우선) 을 먼저 실행합니다.
