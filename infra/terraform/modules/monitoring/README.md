...
- **EC2 CPU** — t3.small 은 크레딧 기반이라 CPU 100% 자체가 사고가 아니다. 이 앱은 22:25 몇 초를 빼면 사실상 유휴라 크레딧이 마를 일이 없다.
...
가장 중요하고 가장 까다로운 알람이다. 앱은 22:25 배치가 끝나면
...
- 발송 성공한 날 → 22:25 `ALARM→OK`, 22:30 `OK→ALARM`. 매일 알림 2통. 소음.
...
```
어제 22:25 하트비트 → [22:25,22:30) 구간
                       └ 오늘 22:30 에 24시간 창 밖으로 밀려난다
  · 오늘 22:25 발송 성공 → 밀려나기 5분 전에 오늘 것이 들어옴 → OK 유지, 알림 없음
  · 오늘 발송이 안 돎    → 창이 완전히 비는 순간 ALARM 전이
                          → 22:30~22:10 KST 메일 (통금 22:30 까지 20분 이상 여유)
```
...
2. 판정이 22:35 / 22:45 재시도보다 이르다. **의도한 것이다.** `DispatchCompleted` 는
   "22:25 배치가 돌기는 했는가"의 하트비트이지 "전원에게 도착했는가"가 아니다(후자는
   `DispatchFailures` 알람이 본다). 22:30 에 하트비트가 없다 = 배치 자체가 안 돌았다 =
...
```
Namespace  : Imlate
Metric     : DispatchCompleted   Unit=Count  Value>=1   (22:25 배치 종료 시 1회)
